...
{
    public static List<T> FindObjectsOfTypeAll<T>(bool includeInActives = true)
    {
        ...
        for (int i = 0; i < UnityEngine.SceneManagement.SceneManager.sceneCount; i++)
        {
            ...
            if (s.isLoaded)
            {
                ...
                for (int j = 0; j < allGameObjects.Length; j++)
                {
                    ...
                    results.AddRange(go.GetComponentsInChildren<T>(includeInActives));
                }
            }
        }
        return results;
    ...
    public static IEnumerable<Transform> GetChildsRecursive(this Transform transform)
    {
        for (int i = 0; i < transform.childCount; i++)
        {
            foreach (var item in transform.GetChild(i).GetChildsWithOwnRecursive())
            {
                yield return item;
            }
        }
    ...
    static IEnumerable<Transform> GetChildsWithOwnRecursive(this Transform transform)
    {
        yield return transform;

        for (int i = 0; i < transform.childCount; i++)
        {
            foreach (var item in transform.GetChild(i).GetChildsRecursive())
            {
                yield return item;
            }
        }
    ...
    {
        foreach (var item in transform.GetChildsRecursive())
        {
            if (item.name == name)
            {
                return item;
    ...
    public static Transform FindChildRecursive(this Transform transform, Predicate<Transform> pred)
    {
        foreach (var item in transform.GetChildsRecursive())
        {
            if (pred(item))
