...
public static partial class ArikanExtensions
{
    ...
    /// <returns></returns>
    public static bool CanSee(this Collider lookingObj, GameObject target, int layerMaskValue = 1)
    {
        ...
        return isHit && (hit.collider != null && hit.collider.gameObject == target);
    }
    ...
    /// <returns></returns>
    public static bool CanSee(this Collider lookingObj, GameObject target,float colliderOutsideMagnitude, int layerMaskValue = 1)
    {
        Vector3 toTargetDirection = (target.transform.position - lookingObj.transform.position).normalized;
